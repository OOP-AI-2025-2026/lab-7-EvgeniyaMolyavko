...
`Function` - це вбудований функціональний інтерфейс, який був доданий у версії Java SE 8 до пакету `java.util.function`. 
Інтерфейс приймає значення одного типу у якості аргументу та повертає значення іншого типу.
...
Даний інтерфейс часто використовується для перетворення одного значення в інше. В інтерфейсі описаний один абстрактний метод `apply()`, 
який приймає об'єкт одного типу та повертає об'єкт іншого (типи вхідного та вихідного об'єктів можуть бути різними або співпадати).
